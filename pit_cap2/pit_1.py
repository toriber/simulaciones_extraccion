...
def g_func(point):
    value=abs(point[0])+ (5-point[1])-2
    return value

def gamma_sq(x0,pit):
    Gamma=np.array(list())
    for point in pit:
        if 3*abs(point[0]-x0[0]) +x0[1] <=point[1]:
...
if Path("c_pit2.csv").is_file():
    print ("File c_pit2.csv exist")
    c_new=np.array(pd.read_csv("c_pit2.csv",sep=",",header=None))
...
prob+= (
    lpSum([(h**2)*p_var[i] for i in p] ) <= CAPACIDAD
        )
